...
def makeUpdate(container, config):
    cmdToRun = "updater.phar --no-interaction"

    print(f"Now running the command `{cmdToRun}` in the nextcloud container!! This may take a while..")
    container.exec_run(cmd=cmdToRun)
    print(f"FINISHED: with updating Nextcloud. See you at the next scheduled time. ;)")
...
def startMainProcess():
    print("\n\nCOMMENCING THE SCHEDULED TASK TO UPDATE NEXTCLOUD..\n")
    container, config = containerChecks.mainChecks()
    makeUpdate(container, config)
